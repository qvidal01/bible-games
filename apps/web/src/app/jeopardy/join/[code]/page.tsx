...
export default function JoinJeopardyPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      router.push(`/jeopardy/game/${code.toUpperCase()}`);
    } catch (err) {
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-gradient-to-b from-blue-900 via-blue-800 to-blue-900 flex flex-col items-center justify-center p-4">
        <div className="text-center mb-8">
          ...
          <p className="text-blue-200">Room Code: <span className="text-yellow-400 font-bold text-2xl">{code.toUpperCase()}</span></p>
        </div>
            ...
            <div className="space-y-4">
              {roomInfo && (
                <div className="bg-blue-900/50 rounded-lg p-4 mb-4">
                  ...
                  <p className="text-blue-400 text-sm">{roomInfo.playerCount}/{roomInfo.maxPlayers} players</p>
                </div>
              )}
