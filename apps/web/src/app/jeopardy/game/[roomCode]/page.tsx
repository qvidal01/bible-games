...
export default function JeopardyGamePage({ params }: { params: Promise<{ roomCode: string }> }) {
  ...
  }, [roomCode, router, setRoomCode, setHostId, addPlayer, checkAndRegisterConnection, enableTeamMode, createTeam, setStatus]);
  ...
  // Handle reveal answer (host-only by default)
  const handleRevealAnswer = (showToAll: boolean = false) => {
    setShowAnswer(true);
    // Only broadcast to other players if explicitly requested
    if (showToAll) {
            ...
            <div className="flex items-center gap-4">
              ...
              {isHost && !isStudyMode && (
                <button
                  ...
                  title="Open Projector View (for TV/Zoom screen share)"
                  aria-label="Open projector view in new window"
                >
